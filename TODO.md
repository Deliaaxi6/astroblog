...
| `8cb7990` | Gitalk OAuth 凭证配置（repo AstroBlog / owner Deliaaxi6，待本地实测） |
| `f086c20` | 音乐歌单管理台可编辑（新增 src/data/music.ts 唯一数据源 + 第七 tab） |
| `7dd18ff` | AI 可做项收尾：清理 site.ts 死字段 cloudMusicIds、文章页封面 preload |
| `36e1bda` | 【原创设计】纸墨×毛玻璃全局设计系统（朱砂红强调、墨纸暗色、3 固定光斑背景、玻璃导航/卡片） |
| `bc8b9e8` | 【原创设计】首页重构：杂志目录式文章列表（编号+日期）+ 两列布局（说说/画廊/播放器）+ 底部主题卡+仪表盘，移除原版 hero 名片与演示 toast |
| `f7216c1` | 【原创设计】文章列表/标签页/时间线/文章页纸墨化（目录式玻璃卡、阅读区纯纸底、暖色代码块） |
| `4fe0b90` | 【原创设计】全站清除靛紫硬编码，统一替换为朱砂红/琥珀暖色系（8 文件） |
| `0144f94` | 【原创设计】关于页纸墨页头（朱砂细线上标 + 分隔线） |
| `4091c92` | 【原创设计·档案手账（定稿）】批次①全局+首页：档案色板（冷灰纸/墨/荧光黄/印章红/钢蓝）、纸面卡片、导航档案线、底部迷你播放条、首页重构（刊头+卷宗抬头条+荧光头条+极简文章行+存档说说+侧栏）+ 站名占位「纸上拾光」 |
| `ae9ba45` | 【原创设计·档案手账】批次②列表页：文章/标签卡片档案抽出 hover（印章左边框）、衬线标题、文章页纸面背景、timeline/tags 清琥珀换档案色 |
| `f7433f4` | 【原创设计·档案手账】批次③剩余页清色：moments/photowall/projects/friends/about/404 琥珀硬编码 → 印章红/钢蓝/冷墨（dist 全站 CSS 旧色零残留） |

> 另：2026-08-16 恢复 `moment-1777182725435.md`（工作区再次被外部因素删除，经 git blob 无损恢复，含 3 个提交，未计入上表）
...
原四项（说说轮播真实数据 / 编辑器 MD 预览 / 播放器回退 / 示例文章扩充）中前三项已完成；可选优化中 AI 可自主项（description 核查、字体子集化评估、死字段清理、封面预加载）也已全部收尾：

| 事项 | 结论 |
|---|---|
| 示例文章内容扩充 | 需用户提供素材/方向，AI 无法凭空代写 |
| 逐篇完善 description | 已核查 4 篇，全部真实合理，无需修改 |
| 字体子集化 | 站点用系统字体栈（Noto Serif SC 等），无 web font 加载，无需子集化 |
| 图片懒加载 | 已覆盖（上一轮） |
| site.ts 死字段 | cloudMusicIds 已删除，播放器空态文案同步更新 |
| 封面预加载 | 4 篇文章页已加 `<link rel="preload" as="image">`（首屏 LCP 图） |
| R2 用量监控/防盗链 | 依赖 R2 凭证，凭证未填无法进行 |
| 评论备份策略 | Gitalk 数据天然存于 GitHub Issues，自带版本备份，无需额外策略 |

## 五、后续规划（可选优化）

- 持续写作真实内容（替换示例文章）⬅ 需用户提供素材/方向
- 友链 / 项目 / 相册内容真实化（当前为原版演示数据）⬅ 需用户提供真实信息
- 音乐歌单自定义（`cloudMusicIds` 目前用原版 3 首，可做成管理台可编辑）✅ 已完成（管理台"音乐"tab，数据源 src/data/music.ts）
- SEO 已深化（canonical/JSON-LD/预连接/封面 preload），description 已核查，剩余：线上验证实际收录效果
- 性能：懒加载已覆盖；字体为系统栈无需子集化；剩余可选：图片 CDN 策略（依赖 R2）
- R2 用量监控 / 防盗链配置（依赖 R2 凭证）
- 评论数据备份策略（Gitalk 数据在 GitHub Issues，天然有备份，无需额外做）
...
### 环境信息
- Git：user `Deliaaxi6` / `deliaaxi6@gmail.com`，分支 `main`，共 32 个提交，无远程
- 设计定稿：**档案手账**（冷灰纸 #f4f3ef / 墨 #23232b / 荧光黄 #e8d24a / 印章红 #b02e26 / 钢蓝 #31577a；暗色=冷墨纸），基于官方 frontend-design skill 规避三种 AI 模板风格后定稿（此前「纸墨×毛玻璃」与「编辑杂志流」均被否决）
- 待定：站名占位「纸上拾光」（首页刊头 + 导航 + OG/JSON-LD），用户确认后替换
